...
/// <summary>
/// Gravity calculation implementation from the "Something Fancy" section of <see href="http://gyrowiki.jibbsmart.com/blog:finding-gravity-with-sensor-fusion">GyroWiki's article</see>.
/// All settings can be left at default, and should not be user facing.
/// </summary>
public class JibbGravityCalculator : IGravityCalculator
{
	...
	// adapted from the gyrowiki article, in combination with the implementation from GamepadMotionHelpers
	...
	public Vector3 Update(Vector3 gyro, Vector3 accelerometer, float deltaTime)
	{
		...
		if (accelMagnitude > 0f)
		{
			...
			float smoothFactor = MathHelper.Exp2(-deltaTime / SmoothingHalfTime);
	...
	{
		float correctionRate = MathHelper.Remap(shakiness, ShakinessMinThreshold, ShakinessMaxThreshold, CorrectionStillRate, CorrectionShakyRate);
		...
		{
			correctionRate = MathHelper.Remap(gravToAccelLength, CorrectionGyroMinThreshold, CorrectionGyroMaxThreshold, correctionLimit, correctionRate);
		}
